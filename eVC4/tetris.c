...
#define BufferLength	2940	// This will be 15fps
...
LRESULT CALLBACK WndProc (HWND hwnd,UINT message,WPARAM wParam,LPARAM lParam){
	...
	static SCROLLINFO vsi={sizeof(SCROLLINFO),SIF_DISABLENOSCROLL|SIF_PAGE|SIF_POS|SIF_RANGE,0,271,16,256,0};
	...
	case WM_CREATE:
		...
		CommandBar_InsertMenubarEx(hCmdBar, hInst, TEXT("Res_Menu"), 0);
		CommandBar_Show(hCmdBar, TRUE);
		height=clheight+GetSystemMetrics(SM_CYCAPTION)+CommandBar_Height(hCmdBar)+2;
	...
	case WM_PAINT:
		...
		redraw();
		BitBlt(hdc,0,CommandBar_Height(hCmdBar),clwidth,clheight,hdcMem,0,0,SRCCOPY);
		EndPaint(hwnd, &ps);
		return 0;
		}
	case WM_MOUSEWHEEL:
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int iCmdShow){
	...
	if (!RegisterClassW(&wndclass)){
		MessageBoxW(NULL,szErr_RegisterClass,szAppName,MB_ICONERROR);
		return 0;
	}
	...
	height=clheight+GetSystemMetrics(SM_CYCAPTION)+24;
	hwnd=CreateWindowW(szAppName,szTitle,WS_OVERLAPPED|WS_CAPTION|WS_SYSMENU|WS_MINIMIZEBOX|WS_VSCROLL,(cxScreen-width)/2,(cyScreen-height)/2,width,height,NULL,NULL,hInstance,NULL);
